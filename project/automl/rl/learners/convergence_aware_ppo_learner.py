...
class ConvergenceAwarePPOLearner(PPOLearner):
    ...
    parameters_signature = {
                        ...
                        "kl_divergence_memory" : ComponentInputSignature(default_component_definition=(
    ...
    def initialize_kl_divergence_memory(self):
        ...
        self.kl_divergence_memory = self.get_input_value("kl_divergence_memory")
        self.kl_divergence_treshold = self.get_input_value("kl_divergence_treshold")

        self.lg.writeLine(f"Convergence will be noted with kl divergence, using an average of {self.kl_divergence_memory.capacity} values, that must be bellow {self.kl_divergence_treshold}")

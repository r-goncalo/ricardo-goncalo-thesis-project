...
class HyperparameterOptimizationPipelineParallel(HyperparameterOptimizationPipeline):
    
    parameters_signature = {
        "runs_per_configuration" : InputSignature(default_value=3)                        
                       }
    ...
    def _proccess_input_internal(self): # this is the best method to have initialization done right after
        ...
        self.runs_per_configuration = self.get_input_value("runs_per_configuration")
    ...
    def _create_components_to_optimize(self, trial : optuna.Trial):
        ...
        trial_name = self.gen_trial_name(trial)  

        trial_folder = self.open_or_create_relative_folder(trial_name)
        ...
        for i in range(self.runs_per_configuration):
            ...
            name = f"run_{i}"
            component_to_opt.pass_input({"name" : name, "base_directory" : trial_folder, "create_new_directory" : False})  

            components_to_return.append(component_to_opt.get_artifact_directory())
        
            self.lg.writeLine(f"Created component with name {name} in trial {trial_name}")
        
        self.trial_components[trial.number] = self._create_components_to_optimize(trial)    
    
    
    def _get_components_to_test(self, trial : optuna.Trial) -> list[str]:
        
        if not trial.number in self.trial_components.keys():
            self._create_components_to_optimize(trial)
            
        
        return  self.trial_components[trial.number]
    ...
    def _try_run_single_step_of_objective(self, trial : optuna.Trial, component_to_test : Component_to_opt_type, step, input_to_pass_before_running : dict):
    ...
    def objective(self, trial : optuna.Trial):
        ...
        for step in range(self.n_steps):
            ...
            evaluation_results = self._try_run_single_step_of_objective(trial, component_to_test, step, input_to_pass_before_running)
